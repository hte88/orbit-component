...
function getItemStyle(index: number, totalItems: number, item: Stacks) {
  const orbitRadius =
    (props.orbitSettings.diameter -
      props.orbitSettings.spacing * props.levelIndex) /
    2
  ...
  >
    ...
    <div class="relative h-full w-full" :style="rotationStyle">
      <template v-for="(item, index) in levelData" :key="index">
        <div
          class="absolute flex h-11 w-11 cursor-pointer items-center justify-center rounded-full border-2 border-gray-800 hover:border-white transition-all duration-300 hover:scale-110 hover:z-10"
        ...
        >
          ...
          >
            <UIcon :name="item.icon" :class="[item.ui.color, 'z-10 h-8 w-8']" />
          </div>
        </div>
      </template>
    </div>
